export const cards = [
  {
    emoji: "🛡️",
    titulo: "FGTS",
    texto: "O patrão deposita 8% do seu salário todo mês. Você pode sacar na demissão sem justa causa.",
    legal: "Lei nº 8.036/1990, Art. 15 e CF/88 Art. 7º, inciso III",
  },
  {
    emoji: "🏥",
    titulo: "Seguro Desemprego",
    texto: "Se for demitido sem justa causa, você pode ter direito ao seguro desemprego — entre 3 e 5 parcelas, conforme o tempo trabalhado e o número de vezes que já solicitou o benefício.\nSolicite entre 7 e 120 dias após a demissão — fora desse prazo o direito é perdido.",
    legal: "Lei nº 7.998/1990 e CF/88 Art. 7º, inciso II",
  },
  {
    emoji: "🦺",
    titulo: "Segurança e Saúde no Trabalho",
    texto: "O empregador é obrigado a:\n- Fornecer EPIs gratuitamente\n- Manter ambiente de trabalho seguro\n- Realizar exames médicos periódicos\n- Treinar os trabalhadores para funções com risco\nVocê pode recusar trabalho em condições inseguras sem punição.",
    legal: "CLT Art. 154 a 201 e Normas Regulamentadoras (NRs) do Ministério do Trabalho",
  },
  {
    customComponent: "CardNR10",
  },
  {
    emoji: "🏥",
    titulo: "Acidente de Trabalho",
    texto: "Se você sofrer acidente durante o trabalho ou no trajeto casa-trabalho-casa, tem direito a:\n- Afastamento remunerado pelo INSS\n- Estabilidade de 12 meses após o retorno ao trabalho — não pode ser demitido nesse período\nComunique imediatamente ao empregador e exija a CAT (Comunicação de Acidente de Trabalho).",
    legal: "Lei nº 8.213/1991 Art. 19 a 23 e CLT Art. 118",
  },
  {
    emoji: "🛡️",
    titulo: "Assédio Moral",
    texto: "Você tem direito a um ambiente de trabalho digno e respeitoso. Assédio moral é qualquer conduta que cause constrangimento, humilhação ou danos psicológicos.\nSe sofrer assédio moral:\n- Registre tudo por escrito\n- Guarde provas (e-mails, mensagens)\n- Comunique ao RH ou sindicato\n- Procure o Ministério do Trabalho",
    legal: "CF/88 Art. 1º, inciso III e Art. 5º — Dignidade da pessoa humana",
  },
  {
    emoji: "⚠️",
    titulo: "Trabalho Informal: conhecer para se proteger",
    texto: "Trabalhar sem registro é comum na área elétrica, mas tem riscos sérios: sem FGTS acumulado, sem seguro-desemprego se dispensado, sem cobertura em caso de acidente de trabalho. Se isso acontecer com você, procure o CRAS ou o sindicato da categoria.",
    legal: "CLT Art. 47 — multa ao empregador por não registro | Lei nº 8.213/1991 — cobertura previdenciária vinculada ao registro formal",
  },
];